...
The modules also need to evolve independently of the teams that consume them. A bug fix in the GKE module should not require changes to Corpus or Pneuma source code — only a version pin update.
...
Modules are decomposed into two groups reflecting their infrastructure layer:
...
- **Inline modules within each team repo** — Rejected. Prevents reuse across teams and creates divergent implementations of the same patterns. A security fix would need to be applied in multiple places.
...
- Consumers must coordinate module upgrades across teams when breaking changes are released — mitigated by semver signalling and the upgrade order convention (core-helpers → other arche modules → consumers)
