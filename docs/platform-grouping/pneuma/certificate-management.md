...
- **istio-csr**: The cert-manager Istio Certificate Signing Request agent replaces istiod's built-in Citadel CA — the component responsible for signing certificate signing requests (CSRs) for workload identities requested by `ztunnel` in the ambient mesh. All workload mTLS certificates are signed by cert-manager through this integration; istiod continues to handle config distribution, service discovery, and control-plane coordination without per-pod sidecars.
...
| `istio-ca-issuer` | cert-manager Issuer backed by the `istio-ca` Secret; cert-manager-istio-csr uses this issuer to sign ambient workload identity certificates requested by `ztunnel` |
| `istio-intermediate-ca` | cert-manager Issuer in `istio-system` backed by the root CA Secret; issues the `istio-ca` intermediate Certificate |
...
| `workload-certificate` | Short-lived mTLS leaf certificate for a workload identity requested by `ztunnel`; issued by cert-manager-istio-csr and automatically rotated by cert-manager |
